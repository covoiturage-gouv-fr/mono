...
description: Generate candidate fixes for verified security findings. Consumes
  TRIAGE.json (preferred) or VULN-FINDINGS.json. Each finding gets a per-finding
  patch subagent + an independent reviewer; output is written as inert diffs for
  human review — the skill never applies a diff. Writes PATCHES/bug_NN/{patch.diff,
  patch_result.json}, PATCHES.md, and PATCHES.json. Use when asked to "fix the
  findings", "patch these vulns", "generate fixes", or "close the loop on
  triage".
...
Fourth and final leg of the find-and-fix loop (`/threat-model -> /vuln-scan ->
/triage -> /patch`; see `SECURITY-REVIEW.md`). Turns a ranked list of verified
findings into candidate diffs.
...
can't be prompt-injected into use. (Per project policy the CLAUDE agent cannot
commit; a human reviews and applies.)
...
- `--repo PATH`: target codebase, read-only (default cwd, e.g. `api/`).
  Required; the skill stops if cited files don't resolve under it.
...
**Do not execute target code.** No building, running, installing, or network.
The oracle here is a fresh-context reviewer reading source, plus a regression
test embedded in the diff for the human to run.
...
`{"phase": 0, "args": {"repo": "", "top": null, "id": null, "findings_path": ""}}`
Then Bash:
`python3 .claude/skills/_lib/checkpoint.py save ./.patch-state 0 args --from ./.patch-state/_chunk.tmp`
...
- If `--top N`: sort by `severity` HIGH > MEDIUM > LOW then `confidence`
  desc, keep the first N.
...
2. ROOT CAUSE FIRST. Trace backward from the cited sink to where the bad
   ...
   For RPC: SQL injection → use the ILOS `sql` parameterized template, not
   string concatenation; cross-tenant/IDOR → add the operator_id scope at
   the repository/query layer; PII exposure → scrub before logging/response.
...
4. MINIMAL DIFF. Smallest change that fixes the root cause. No refactoring,
   ...
   surrounding code's style (naming, error handling, ILOS conventions).
...
6. REGRESSION TEST. As part of the diff, add ONE test case that fails before
   ...
   tests (look for *.spec.ts, *_test.*, tests/, test/). If no test directory
   exists nearby, omit the test and say so in <test_note>.
...
empty, mark `status: "no_patch"`. Otherwise write the diff text to
...
`{"id": "", "bug_nn": "", "status": "", "rationale": ""}`, then Bash:
...
embedded in finding prose (a prompt-injection vector via scanned source)
from reaching both the author and the gate.
...
2. SUPPRESSION. Does the diff fix a root cause, or does it suppress the
   symptom (try/catch that swallows, early-return on a magic value, deleting
   the check that fired, lowering a log level)?
...
finding. Set `verified: "static_review_only"` for every result regardless of
...
> re-attacked. Read each diff yourself, run the embedded regression test, and
> apply by hand — see `SECURITY-REVIEW.md`.
...
These are drafts. Review and run the embedded tests before applying — see SECURITY-REVIEW.md.
```
...
Static mode against `api/` after a triage run:

```
/vuln-scan api/ --single
/triage api/VULN-FINDINGS.json --repo api/ --auto
/patch TRIAGE.json --repo api/ --top 3
```

Expected: up to three diffs under `PATCHES/bug_00..02/`, each
`verified: "static_review_only"` with an ACCEPT/REJECT review. Confirm
nothing was written into `api/`.
...
- **The diff carries a regression test** rather than running it. The skill
  cannot execute target code; the test is for the human who applies the diff.
- **Reviewer never sees finding prose.** Target source can contain injected
  instructions that survive into a scanner's `description` field. The patch
  author sees that prose (it has to, to know what to fix); the reviewer
  doesn't, so injected text cannot pass its own gate.
- **`verified: static_review_only`** means "an agent read it" regardless of
  ACCEPT/REJECT. There is no execution oracle in this repo by design.

## Provenance

Ported from `anthropics/defending-code-reference-harness` (`patch` skill),
static mode only. The execution-verified `vuln-pipeline` delegation was
removed (no autonomous sandbox pipeline in this repo). RPC retune: root-cause
hints reference ILOS `sql` parameterization, operator-id tenant scoping, and
PII scrubbing. See `SECURITY-REVIEW.md`.
