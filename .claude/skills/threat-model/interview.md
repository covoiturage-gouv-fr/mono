...
Prefer `AskUserQuestion` to put the choices in front of the owner; batch up to
four questions per call, and always allow free-text via "Other".
...
look for route definitions (ILOS `@handler`, Express, Next.js handlers),
file-open calls, socket listeners, deserializers, `Deno.env` reads. Where code
confirms the owner, tag `[Code-verified]`. Where code shows an entry point the
owner did not mention, ask about it: "I see an `/admin` route in
`routes.ts:88`; is that reachable in production?"
...
a generic checklist. Name classes at the granularity of "IDOR on operator-
scoped journeys" or "SQL injection in raw query construction", not "web
vulnerabilities".
...
deserialization, auth, crypto, supply chain, infra/IAM) didn't make either
list, ask the owner why before dropping it.
...
Phrase the threat at the level where it survives a patch.
...
deprioritized threats with a recorded reason are a valid output.
...
effort`. Prefer controls that survive the next bug (parameterized queries,
central tenant guards, scoped short-lived tokens, CSP, allocation caps) over
patches for the last one.
