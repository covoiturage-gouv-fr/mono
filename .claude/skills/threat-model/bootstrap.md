...
JavaScript/TypeScript, or polyglot. For RPC the relevant stacks are Deno/TS
(ILOS api), Next.js (frontends), and SQL (sqlmesh). Do not build, run, or fuzz
the target. The Bash tool is permitted **only** for `git` (history mining),
`find`/`ls` (layout), `gh api` (public advisory lookup), and `cat` (re-reading
skill files). Do not execute anything from inside `<target-dir>`. The same
...
- `<target-dir>` (required): local checkout (e.g. `api/`).
...
| Agent | Brief | Returns |
|---|---|---|
| **Docs reader** | Read `README*`, `SECURITY.md`, `CHANGELOG*`, top-level `docs/`, `CLAUDE.md`, and the build manifest (`deno.json` / `package.json` / `pyproject.toml`). Summarize what the project says it is, who uses it, and any security claims or fix entries it documents. | Prose system description; list of self-documented security fixes. |
...
| **Infra reader** | Read deploy-time config: `*.tf`/`*.tfvars`, k8s manifests, `Dockerfile*`, `docker-compose*.yml`, CI workflows, and any IAM/service-account/dataset-ACL files. For each, name (a) the identity it runs as and what that identity can reach, (b) any access grant not managed in this tree, (c) credentials or principals that survive a migration or teardown. | Candidate section 3 rows for infra surfaces + candidate section 4 rows: `{threat, surface, asset}` where the config itself is the finding. |
| **Asset finder** | Identify what the code protects or produces: sensitive data it reads/writes (secrets, keys, user records, PII, DBs), process integrity, service availability, and downstream embedder assets if it's a library. | Candidate section 2 rows: `{asset, description, sensitivity}`. |
...
| Surface | Look for |
|---|---|
| Network | socket `listen`/`accept`/`bind`; HTTP route definitions (ILOS `@handler`, Express routes, Next.js route handlers); RPC/GraphQL service defs |
| File / format parsing | file-open calls; format magic-byte checks; "parse"/"decode"/"load"/"unmarshal" function names |
| CLI / env | argv parsers; env reads (e.g., `Deno.env.get`, `getenv`) |
| Deserialization | language-native deserializers on external data (e.g., `JSON.parse` on untrusted bytes, YAML loaders, `pickle`) |
| DB / query | raw query-string construction; ORM `.raw()`/`.query()` escapes; concatenation into the ILOS `sql` helper |
| IPC / plugins | dynamic load; subprocess spawn; `eval`/`exec` on config; dynamic import |
| Supply chain | dependency lockfiles; vendored libs; `curl \| sh` in build scripts |
| Infra / IAM | terraform `*_iam_*`; k8s `serviceAccountName`; secrets mounts; docker-compose env/secrets |
...
rows. Dedupe, name the trust boundary for each ("unauth HTTP → application
logic", "authenticated-untrusted operator → proof store"), and for each list
which section 2 assets are reachable from it. Supply-chain, build-time, and
infra/IAM surfaces **are** entry points even though no runtime input crosses
them. **Every row here must get at least one threat in Stage 3 or 4**; that's
the coverage invariant the emit-time check enforces.
...
entry point it traversed; read the relevant source to confirm. If a vuln's
entry point isn't in section 3, the Surface mapper missed one; add it now. Hold
this table in your working notes; it does **not** go into `THREAT_MODEL.md`
verbatim. It becomes the `evidence` column in Stage 3.
...
- 3 IDOR bugs on different operator-scoped resources → **one** threat:
  "Cross-tenant data leakage via missing operator scoping". Evidence: all 3.
...
weren't in the vuln list. Grep for the same pattern (other endpoints calling
the same unsafe helper, other queries missing a tenant filter). You are not
trying to prove these are exploitable; you are estimating how much of the
surface shares the pattern. More siblings → higher likelihood.
...
For each cluster, assign `actor`, `impact`, `likelihood`, `controls`, `status`
per `schema.md`, and a `recommended_mitigation` working note (one class-level
control that survives the next bug: "parameterized queries everywhere",
"central tenant-scope guard", "PII-aware logging filter"). These become
section 8 rows in Stage 5.
...
For **infra/IAM entry points**, walk over-grant, lateral identity, drift,
residual access, column exposure, and scope enforcement instead.
...
(deployment context, intended actors, controls you couldn't verify, risk
appetite). These seed a later `/threat-model interview --seed THREAT_MODEL.md`
pass and the first `/vuln-scan` focus areas.
...
it `closes_class` (yes/partial), and a rough `effort` (S/M/L).
...
`<target-dir>/THREAT_MODEL.md` in one Write.

1. Write tool → `./.threat-model-state/THREAT_MODEL.md` (clobbers any prior
   file) containing only the title line and `## 1. System context` section.
2. For each remaining section, Write tool → `./.threat-model-state/_chunk.tmp`
   containing that ONE section's markdown, then Bash:
