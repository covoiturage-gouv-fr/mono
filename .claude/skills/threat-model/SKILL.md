...
name: threat-model
description: >-
  Build a threat model for a target codebase (RPC: api/, frontends, sqlmesh).
  Three modes: "interview" walks an application owner through the four-question
  framework and produces a threat model from their answers; "bootstrap" derives
  a threat model from the code plus past vulnerabilities (CVEs, git history,
  pentest reports) when no owner is available; "bootstrap-then-interview" chains
  the two when both owner and codebase are present. All write THREAT_MODEL.md in
  a shared schema. Use when asked to "threat model", "build a threat model",
  "map the attack surface", or "what should we be worried about in this
  codebase".
...
bug has been found yet. It is the map; vulnerability discovery (`/vuln-scan`) is
the metal detector. A good threat model tells `/vuln-scan` where to look and
tells `/triage` which findings matter.

**Litmus test:** If patching one line of code makes an entry disappear, it was
a vulnerability, not a threat. A threat ("cross-tenant data leakage via missing
operator scoping") still stands after every known bug is fixed; a vulnerability
("`journeys.ts:88` drops the `operator_id` filter") does not. This skill
...
This skill is the first leg of the find-and-fix loop documented in
`SECURITY-REVIEW.md`: `/threat-model -> /vuln-scan -> /triage -> /patch`.
...
If the user asks you to validate a threat by running an exploit, decline: this
harness is static-only by design (see `SECURITY-REVIEW.md`). Recommend a
human-built proof-of-concept in a controlled environment instead.
...
so downstream consumers (`/vuln-scan`, `/triage`) do not need to know which
mode produced it.
...
For an `api/`-scoped model, write to the repo-root `THREAT_MODEL.md` (the
committed, canonical location for RPC) rather than `api/THREAT_MODEL.md`,
unless the user asks for a per-component file.
...
3. For `bootstrap`: any open questions the code could not answer (these seed a
   later `interview` pass and the first `/vuln-scan` focus areas).
4. For `interview`: any owner statements that could not be verified in code
   (these seed follow-up code review and `/vuln-scan`).

---

## References

- `SECURITY-REVIEW.md` (repo root) for the engagement context, the full
  find-and-fix loop, and how this skill relates to `/check-security`.
- The existing `.claude/skills/check-security/SKILL.md` checklist for the RPC-
  specific vulnerability classes the threat model should anticipate.
