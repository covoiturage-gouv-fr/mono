...
description: Triage a batch of raw security findings. Verify each is real,
  ...
  owner. Takes a directory or file of scanner output (e.g. VULN-FINDINGS.json)
  and writes TRIAGE.json + TRIAGE.md sorted by what actually needs engineering
  attention. Use when asked to "triage findings", "validate scanner output",
  "prioritize vulns", or "review the backlog". Runs interactively by default;
  pass --auto to skip the interview.
...
short, ranked, owned list instead of a raw dump. Third leg of the
...
- findings path (first positional, required): a JSON file, a directory of
  JSON files, a `VULN-FINDINGS.json`, or a markdown report.
...
- `--repo PATH`: path to the target codebase, read-only (default cwd, e.g.
  `api/`). Verification needs source access; the skill stops with an error if
  the cited files aren't reachable.
- `--fp-rules FILE`: append the contents of FILE to the verifier's
  exclusion-rule list (Phase 3a). Use for org-specific precedents: "operator
  data is always tenant-scoped via repository X — raw-query SQLi only", "infra
  rate limits cover DoS", etc. Plain text, one rule per line or paragraph.
...
1. **Environment & trust boundary** (header `Environment`, single-select)
   ...
   Reachability is judged against this boundary. For RPC `api/`, the default
   is "Internet-facing, multi-tenant: authenticated operator payloads are
   untrusted."
...
2. **Threat model** (header `Threat model`, multi-select)
   ...
   `Mass PII exfiltration (RGPD)`, `Operator token theft / impersonation`,
   `Incentive fraud via fabricated proofs`.
   Phase 4 boosts findings that map onto a stated threat. If a root
   `THREAT_MODEL.md` exists, read its section 4 and offer its top threats
   as the defaults here.
...
- Environment: `Internet-facing, multi-tenant. Treat any externally-reachable
  entry point and any authenticated operator payload as untrusted; flag
  trust-boundary assumptions explicitly in rationale.`
- Threat model: from the root `THREAT_MODEL.md` section 4 if present, else empty.
...
| `category`      | `type`, `cwe`, `rule_id`, `vulnerability_class`          |
...
repo's own basename, e.g. `api/src/foo.ts` with `--repo api`).
...
- A missing global protection (auth check, output encoding, tenant filter)
  reported once per endpoint that lacks it
- A cause ("missing input validation on `name`") and its consequence
  ("SQL injection via `name`") in the same code path

Treat as DISTINCT:
- Different categories in the same file region (an "ssrf" near an
  "idor-cross-tenant" is not a duplicate just because the lines are close)
...
CANDIDATES:
{one line per surviving finding: "f003 | api/src/auth.ts:112 | sql-injection | User lookup concatenates name into query"}
```
...
```json
{"phase": 2, "context": {}, "findings": [], "candidates": ["f001", "f003"]}
```
...
{context.environment or "Internet-facing, multi-tenant. Authenticated operator payloads are untrusted."}
...
3. HUNT FOR PROTECTIONS.
   ...
   - Framework auto-escaping, ILOS `sql` parameterized queries, prepared
     statements
   - Operator/tenant scoping applied before the query (operator_id filter)
   - Type constraints (the value is an int, an enum, a fixed-length token)
   - Authentication / authorization gates (middleware) before this path
   - Configuration that limits exposure (feature flag off, debug-only)
   - Dead code, test-only code, migration/fixture code
...
even if technically accurate. Cite the rule number in your verdict.
  ...
  2. Test-only code, dead code, migration/fixture code, or a crash with no
     security impact.
  ...
  4. Memory-safety concerns in memory-safe languages (TS/JS) outside FFI /
     WASM glue.
 ...
 10. Outdated dependency versions (managed by a separate process: npm/deno
     audit + CI).
 ...
 14. XSS in a framework with default auto-escaping (React, Next, Angular,
     Vue) UNLESS the sink is a raw-HTML escape hatch
...
depends on runtime configuration you cannot read). Use sparingly; it must
...
(3) Hunt for protections: validation, ILOS sql/parameterized query, tenant
scope (operator_id), type bounds, auth gates, dead/test/migration code.
(4) Stress-test each protection on every path.

Exclusion rules (FALSE_POSITIVE if matched): 1 volumetric DoS;
2 test/dead/migration code; 3 intended design; 4 memory-safety in safe
lang outside FFI/WASM; 5 SSRF path-only; 6 LLM prompt input;
...
verifier's text**, the runtime backgrounded it. Pick one recovery and use it
for the whole batch:
...
```json
{"phase": 3, "context": {}, "findings": [], "confirmed": ["f001"]}
```
...
`progress.json:shards_done` (default `[]`), loads the corresponding
...
fixtures or migration/dev-only config? Score in -5..+5:
...
Attach as `owner_hint`. State the source so confidence is clear. For
non-true-positive findings, set `owner_hint: null`.

**Checkpoint:** Write tool → `./.triage-state/_chunk.tmp`:

```json
{"phase": 5, "context": {}, "findings": []}
```
...
Next step: `> /patch ./TRIAGE.json --repo <repo> --top 5`

---

## Testing this skill

Run `/vuln-scan api/ --single` on a small scope to produce a
`VULN-FINDINGS.json`, then:

```
/triage api/VULN-FINDINGS.json --repo api/ --auto --votes 1
```

Expected: `TRIAGE.{json,md}` written; every input finding appears exactly
once (true_positive / false_positive / duplicate); checkpoint resume works
if interrupted. Hand-check a sample of TRUE_POSITIVE/HIGH results (the
`first_links` should point at real call sites) and a sample of
...
- **Checkpoints are per-phase JSON**, not conversation state. File-backed
  checkpoints let a brand-new session pick up from the last completed phase.
  `./.triage-state/` is scratch — it is git-ignored (see `.gitignore`).
...
Ported from `anthropics/defending-code-reference-harness` (`triage` skill),
near-verbatim (the methodology is stack-agnostic). RPC retune: trust-boundary
and threat-model defaults pointed at the multi-tenant operator model;
ASAN/autonomous-pipeline ingest paths dropped (this repo runs static skills
only). See `SECURITY-REVIEW.md`.
