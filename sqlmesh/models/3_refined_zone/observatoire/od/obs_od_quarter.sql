MODEL (
  name refined_zone.obs_od_quarter,
  ...
  cron '@monthly',
  grain [ 'quarter_date', 'type', 'territory_1', 'territory_2'],
  tags ['refined', 'observatoire', 'od_quarter'],
);
...
od_agg AS (
  ...
  -- country
  SELECT
    a.year, a.quarter, 'country' AS type,
...
SELECT
  make_date(a.year, (a.quarter-1)*3 + 1, 1) AS quarter_date,
  a.year,
  a.quarter,
  a.type,
  a.territory_1,
...
@IF(@runtime_stage = 'creating', CREATE UNIQUE INDEX IF NOT EXISTS uq_quarter_date_type_territory_1_territory_2 ON refined_zone.obs_od_quarter (quarter_date, type, territory_1, territory_2));
