MODEL (
  name refined_zone.obs_directions_users_month,
  ...
  cron '@monthly',
  grain ['code','type','month_date','direction'],
  tags ['refined', 'observatoire', 'directions_users_month'],
);
...
WITH journeys_enriched AS (
  SELECT
    ...
    j.driver_id,
    j.passenger_id,
    DATE_TRUNC('month', j.start_datetime_tz)::date AS month_date,
    g.start_epci,
    g.end_epci,
    g.start_aom,
    g.end_aom,
...
@IF(@runtime_stage = 'creating', CREATE UNIQUE INDEX IF NOT EXISTS uq_month_date_code_type_direction ON refined_zone.obs_directions_users_month (month_date, code, type, direction));