MODEL (
  name refined_zone.obs_directions_quarter,
  ...
  cron '@monthly',
  grain [ 'quarter_date', 'code', 'type', 'direction'],
  tags ['refined', 'observatoire', 'directions_quarter'],
);
...
users as (
  SELECT
    ...
    type,
    direction,
    extract('year'  FROM quarter_date)::int AS year,
    extract('quarter' FROM quarter_date)::int AS quarter,
    unique_drivers,
    unique_passengers,
    new_drivers,
    new_passengers
  FROM refined_zone.obs_directions_users_quarter
  WHERE quarter_date >= @start_ts
    AND quarter_date <  @end_ts
)
