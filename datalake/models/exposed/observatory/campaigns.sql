...
-- `raw_zone.campaigns` + jointure `trusted_zone.perimeters_agg` côté API Deno).
--
-- L'API ne lit que la zone exposée. Cette vue :
--   * projette UNIQUEMENT les colonnes du contrat public (celles servies par
--     l'ancien `raw_zone.campaigns` de sqlmesh) — on écarte les colonnes
--     internes du seed brut (email, collectivite, column00, « Mise à jour »,
--     nom_région, siren…) qui ne doivent PAS fuiter sur une API publique ;
--   * renomme `lien_page_collectivité` -> `lien` (contrat historique) ;
--   * embarque la géométrie GeoJSON du territoire (jointure périmètre au
--     dernier millésime).
