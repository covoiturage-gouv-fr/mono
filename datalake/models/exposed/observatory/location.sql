...
-- api-datalake en SQL via `h3_cell_to_parent(<h3index_z8>, n)` (zoom n borné [0,8]),
-- d'où l'exposition directe des index H3 résolution 8 plutôt que des lat/lon :
-- parité H3 exacte pour n <= 8, sans rapatrier les positions.
...
SELECT
    ...
    end_h3index_z8
FROM {{ ref('carpools') }}
WHERE valid_acquisition_status
