{{ config(
    ...
    tags=['refined', 'direction', 'semester_country_from']
) }}

WITH filtered_carpools AS (
  {{direction_filtered_carpools_country(model_column='incremental_date',lookback_nb=0, lookback_unit='semester')}}
)

SELECT
  start_country AS code,
  'country' AS type,
