{% macro direction_model(perim, grain, direction) %}
...
{{ config(
  materialized='view',
  tags=['refined', 'direction', grain, grain ~ '_com_' ~ direction]
) }}

SELECT * FROM {{ ref('direction_' ~ grain ~ '_arr_' ~ direction) }}
UNION ALL
SELECT * FROM {{ ref('direction_' ~ grain ~ '_plm_' ~ direction) }}
...
{{ config(
  ...
  tags=['refined', 'direction', grain, grain ~ '_' ~ perim ~ '_' ~ direction]
) }}
