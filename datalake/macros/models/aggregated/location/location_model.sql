...
exploded AS (
  ...
  FROM filtered_carpools WHERE start_code IS NOT NULL {% if plm_filter %}AND start_code {{ plm_filter }}{% endif %}
  UNION ALL
  ...
  FROM filtered_carpools WHERE end_code IS NOT NULL AND end_code IS DISTINCT FROM start_code {% if plm_filter %}AND end_code {{ plm_filter }}{% endif %}
  UNION ALL
