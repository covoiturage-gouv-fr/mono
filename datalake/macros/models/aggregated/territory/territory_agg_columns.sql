{% macro territory_agg_columns() %}
  ...
  SUM(driver_revenue) FILTER (WHERE is_intra) AS driver_revenue_intra,
  SUM(driver_revenue) FILTER (WHERE NOT is_intra) AS driver_revenue_inter,
  ...
  SUM(passenger_contribution) FILTER (WHERE is_intra) AS passenger_contribution_intra,
  SUM(passenger_contribution) FILTER (WHERE NOT is_intra) AS passenger_contribution_inter,
  SUM(distance) AS distance,
  SUM(distance) FILTER (WHERE is_intra) AS distance_intra,
  SUM(distance) FILTER (WHERE NOT is_intra) AS distance_inter,
  ...
  SUM(oi_amount_operator) FILTER (WHERE is_intra) AS oi_amount_operator_intra,
  SUM(oi_amount_operator) FILTER (WHERE NOT is_intra) AS oi_amount_operator_inter,
  SUM(oi_amount_other) AS oi_amount_other,
  SUM(oi_amount_total) AS oi_amount_total,
  SUM(oi_amount_total) FILTER (WHERE is_intra) AS oi_amount_total_intra,
  SUM(oi_amount_total) FILTER (WHERE NOT is_intra) AS oi_amount_total_inter,
  SUM(
    jsonb_array_length(
